...
namespace QLThuVien.DAO
{
    class Home_DAO : DataProvider
    {
        ...
        {
            string sql = "SELECT TOP 5 NGAYMUON N'Ngày mượn/trả', MAMUONTRA N'Mã mượn trả', MADOCGIA N'Mã độc giả', MANHANVIEN N'Mã nhân viên' FROM dbo.PHIEUMUONTRA ORDER BY NGAYMUON DESC";
            return GetData(sql);
        }
        ...
        {
            string sqlString = string.Format("SELECT TENTACGIA N'Tên tác giả', COUNT(DS.MADAUSACH) N'Số lượng tác phẩm' FROM dbo.SANGTAC ST, dbo.TACGIA TG, dbo.DAUSACH DS WHERE ST.MaDauSach = DS.MaDauSach AND TG.MaTacGia = ST.MaTacGia AND TG.TenTacGia LIKE N'%{0}%' GROUP BY TG.TenTacGia", _timkiem);
            return GetData(sqlString);
        }
        public DataTable SearchDS(string _timkiem)
        {
            string sqlString = string.Format("SELECT DS.TenDauSach N'Tên đầu sách', DS.NamXuatBan as N'Năm xuất bản' FROM dbo.DAUSACH DS, dbo.THELOAI TL, dbo.TACGIA TG, dbo.SANGTAC ST WHERE DS.MaTheLoai = TL.MaTheLoai AND TL.TenTheLoai like N'%{0}%' AND DS.MaDauSach = ST.MaDauSach AND ST.MaTacGia = TG.MaTacGia", _timkiem);
            return GetData(sqlString);
        }
